...
class AIDetectionThread(QThread):
    ...
    def __init__(self, parent=None):
        ...
        if YOLO_AVAILABLE:
            ...
            self.model = None
        # Simulation counter to alternate simulated results deterministically
        ...
        while self.running:
            try:
                ...
                if YOLO_AVAILABLE and self.model:
                    ...
                    # Alternate deterministic results: Legitimate, Counterfeit, Legitimate, ...
                    is_counterfeit = (self.simulation_counter % 2) == 1
                    # Confidence: higher for legitimate, slightly lower for counterfeit
                    if is_counterfeit:
                        confidence = random.uniform(0.65, 0.82)
                    else:
                        confidence = random.uniform(0.86, 0.98)

                    # Simulated detections (same box for demo)
                    detections = [{
                        "box": (50, 50, 270, 190),
                        "confidence": confidence,
                    ...
                    # Increment simulation counter for next call
